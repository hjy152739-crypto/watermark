...
class AdvancedHiddenDecoder(nn.Module):
    ...
    def __init__(self, num_bits, channels=64, img_size=128, architecture='unet', 
        ...
        if use_advanced:
            ...
            # Fall back to original HiddenDecoder (not used in current training, keep placeholder to avoid import)
    ...
    def forward(self, img_w, return_detailed=False):
        ...
        if not self.use_advanced:
            ...
            if self.original_decoder is None:
                raise RuntimeError("Original decoder is not available")
